...
# --- KONFIGURASI ---
# Sesuaikan BASE_DIR sesuai posisi file app.py Anda terhadap folder 'outputs'
...
def main() -> None:
    ...
    with left:
        ...
        scatter = px.scatter(filtered, x="PC1", y="PC2", color="cluster_display", height=520)
        st.plotly_chart(scatter, use_container_width=True)

    with right:
        st.subheader("Komposisi Cluster")
        cluster_counts = filtered.groupby(["cluster", "cluster_label", "cluster_display"], as_index=False).size()
        pie = px.pie(cluster_counts, names="cluster_display", values="size", hole=0.45, height=300)
        st.plotly_chart(pie, use_container_width=True)
        st.dataframe(cluster_counts[["cluster", "cluster_label", "size"]], use_container_width=True, hide_index=True)

    # Lanjutkan bagian sisa visualisasi Anda di bawah sini dengan pola yang sama...
